...
TEST_F(SurfaceTest_UsesSDL, Surface_CreateableFromBMP)
{
	auto s = Surface("P:\\data\\test\\momo.bmp");
}

TEST_F(SurfaceTest_UsesSDL, operatorSDLSurface_ConvertsSuccessfully)
{
	auto s = Surface("P:\\data\\test\\momo.bmp");
	auto& ref = const_cast<SDL_Surface&>(static_cast<const SDL_Surface&>(s));
	Uint8 alpha;

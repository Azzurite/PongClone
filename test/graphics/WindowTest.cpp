...
using WindowParamTuple = std::tuple<Window::State, Window::Fullscreen, Window::Border, bool>;
...
public:
	...
	virtual void SetUp()
	{
		...
		s = std::get<Window::State>(GetParam());
		b = std::get<Window::Border>(GetParam());
...
TEST(WindowConstructTest_UsesSDL, Window_Default)
{
	Window w("Test");
}

TEST_P(WindowConstructTest_UsesSDL, Window_FullScreen)
{
	Window w("TestFullscreen", f, s, i);
}

TEST_P(WindowConstructTest_UsesSDL, Window_Windowed)
{
	Window w("TestWindowed", 50, 50, 100, 100, b, s, i);
}

...
TEST(ColorTest, Color_CreateRGBAColor)
{
	Color c(123, 32, 41, 61);

	EXPECT_EQ(123, c.red());
	EXPECT_EQ(32, c.green());
	EXPECT_EQ(41, c.blue());
	EXPECT_EQ(61, c.alpha());
}

TEST(ColorTest, operatorSDLColor_ConvertsSuccessfully)
{
	auto c = Color(5, 5, 5, 5);

	auto copied = static_cast<SDL_Color>(c);
	decltype(auto) reference = static_cast<SDL_Color&>(c);

	ASSERT_EQ(5, copied.b);
	ASSERT_EQ(5, reference.a);
